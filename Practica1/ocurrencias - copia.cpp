...
int main() {
 ...
 start = clock();
 for (int iteraciones = 0; iteraciones < 4000; iteraciones++)  // Numero de iteraciones se debe ajustar a cada caso
      pos = contar_hasta(Q, 0,fin, b);
 end= clock();
 double dif = end-start;

//cout << "ciclos " << dif << " seg " << dif/(double) (CLOCKS_PER_SEC * 1000.0) << " tama " << fin <<  endl;
